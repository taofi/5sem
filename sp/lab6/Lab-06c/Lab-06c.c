    ...
    do {
        ...
        if (wcscmp(fileOrDirName, L".") != 0 && wcscmp(fileOrDirName, L"..") != 0) {
            ...
            if (findFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
                ...
                swprintf(subDir, MAX_PATH, L"%s\\%s", directory, fileOrDirName);
                ListDirectoryContents(subDir); // Рекурсивный вызов
                printf("\n");
            }
        }
...
void WatchDirectory(const wchar_t* directory) {
    HANDLE hDir = CreateFileW(directory, FILE_LIST_DIRECTORY,
    ...
    while (TRUE) {
        if (ReadDirectoryChangesW(hDir, buffer, sizeof(buffer), TRUE,
            ...
            do {
                ...
                // Если файл был добавлен, мы можем дополнительно проверить, действительно ли он существует
                if (fni->Action == FILE_ACTION_ADDED) {
                    // Проверка существования файла
                    wchar_t newFilePath[MAX_PATH];
                    swprintf(newFilePath, MAX_PATH, L"%s\\%.*s", directory, fni->FileNameLength / sizeof(WCHAR), fni->FileName);
                    WIN32_FIND_DATAW findFileData;
                    HANDLE hFind = FindFirstFileW(newFilePath, &findFileData);
                    if (hFind != INVALID_HANDLE_VALUE) {
                        wprintf(L"Confirmed added file: %s\n", newFilePath);
                        FindClose(hFind);
                    }
                }
