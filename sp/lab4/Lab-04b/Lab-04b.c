#include <windows.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <conio.h>
#pragma warning(disable : 4996)

CRITICAL_SECTION cs;

#define MAX_NUMBERS 1024
#define MAX_PRIMES_SIZE 4096

char globalPrimes[MAX_PRIMES_SIZE];
int globalPrimesSize = 0;

__declspec(thread) char tlsBuffer[1024];

VOID ErrorExit(LPCSTR message)
{
    fprintf(stderr, "%s\n", message);
    ExitProcess(0);
}

int is_prime(int num) {
    if (num < 2) return 0;
    for (int i = 2; i * i <= num; i++) {
        if (num % i == 0) {
            return 0;
        }
    }
    return 1;
}

DWORD WINAPI ThreadFunc(LPVOID lpParam)
{
    int* params = (int*)lpParam;
    int start = params[0];
    int end = params[1];

    int current_buffer_size = 0;

    tlsBuffer[0] = '\0';

    for (int i = start; i <= end; i++) {
        if (is_prime(i)) {
            current_buffer_size += snprintf(tlsBuffer + current_buffer_size, sizeof(tlsBuffer) - current_buffer_size, "%d ", i);
        }
    }

    EnterCriticalSection(&cs);

    _cprintf("Founded primes: %s\n", tlsBuffer);

    if (globalPrimesSize + current_buffer_size < sizeof(globalPrimes)) {
        strcat(globalPrimes, tlsBuffer);
        globalPrimesSize += current_buffer_size;
    }
    else {
        _cprintf("Global buffer overflow\n");
    }

    LeaveCriticalSection(&cs);
    free(params);

    return 0;
}

int compare(const void* a, const void* b) {
    return (*(int*)a - *(int*)b);
}

void sortGlobalPrimes() {
    int numbers[MAX_PRIMES_SIZE];
    int numCount = 0;

    char* token = strtok(globalPrimes, " ");
    while (token != NULL && numCount < MAX_PRIMES_SIZE) {
        numbers[numCount++] = atoi(token);
        token = strtok(NULL, " ");
    }
    qsort(numbers, numCount, sizeof(int), compare);

    globalPrimes[0] = '\0';
    for (int i = 0; i < numCount; i++) {
        char temp[16];
        snprintf(temp, sizeof(temp), "%d ", numbers[i]);
        strcat(globalPrimes, temp);
    }
}

int main(int argc, char* argv[])
{
    if (argc < 4) {
        fprintf(stderr, "usage: %s <process count> <lower_bound> <upper_bound>\n", argv[0]);
        return 1;
    }

    int numberOfThreads = atoi(argv[1]);
    int lower_bound = atoi(argv[2]);
    int upper_bound = atoi(argv[3]);
    DWORD IDThread;
    HANDLE* hThread = (HANDLE*)malloc(numberOfThreads * sizeof(HANDLE));
    if (hThread == NULL) {
        printf("Memory allocation failed\n");
        return 1;
    }

    int i;

    InitializeCriticalSection(&cs);

    int range = upper_bound - lower_bound + 1;
    int step = range / numberOfThreads;

    for (i = 0; i < numberOfThreads; i++)
    {
        int* params = (int*)malloc(2 * sizeof(int));
        params[0] = lower_bound + i * step;
        params[1] = (i == numberOfThreads - 1) ? upper_bound : params[0] + step - 1;

        hThread[i] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ThreadFunc, params, 0, &IDThread);

        if (hThread[i] == NULL)
            ErrorExit("CreateThread error\n");
    }
    Sleep(8000);
    WaitForMultipleObjects(numberOfThreads, hThread, TRUE, INFINITE);
    DeleteCriticalSection(&cs);
    sortGlobalPrimes();
    Sleep(8000);
    _cprintf("\nPrimes array: %s\n", globalPrimes);
    free(hThread);
    return 0;
}