...
struct Student* pSharedStudents = NULL;  // Локальная копия указателя на pStudents

void SetSharedStudents(struct Student* sharedStudents) {
    pSharedStudents = sharedStudents;
}
...
void createSampleFile(const char* filePath) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        snprintf(students[i].ID, sizeof(students[i].ID), "%05d", i + 1);
    }
...
void PrintRow(INT pos) {
    ...
    printf("PrintRow called with position: %d\n", pos);

    // Корректировка позиции
    if (pos < 0) {
        pos = studentCount + pos;
    }
    printf("minus to plus: %d\n", pos);
    ...
    printf("is valid: %d\n", pos);
    printf("pStudents pointer: %p\n", (void*)pStudents);

    if (strlen(pSharedStudents[pos].Name) == 0) {
        printf("Position %d is empty.\n", pos);
        return;
    }
    printf("chto-to: %d\n", pos);

    // Отладочное сообщение о доступе к структуре
    struct Student student = pSharedStudents[pos];
    printf("Accessing student data at position %d:\n", pos);
...
void CloseMapping() {
    UnmapViewOfFile(pSharedStudents);
    CloseHandle(hFileMapping);
    hFileMapping = NULL;
    CloseHandle(hFile);
}
