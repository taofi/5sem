...
void* ThreadFunc(void* lpParam) {
    ...
    char* buffer = (char*)malloc(MAX_NUMBERS * sizeof(char));
    if (buffer == NULL) {
    ...
    memset(buffer, 0, MAX_NUMBERS);
    pthread_setspecific(tlsKey, buffer);

    int current_buffer_size = 0;

    for (int i = start; i <= end; i++) {
        if (is_prime(i)) {
            current_buffer_size += snprintf(buffer + current_buffer_size, MAX_NUMBERS - current_buffer_size, "%d ", i);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    pthread_t* threads = (pthread_t*)malloc(numberOfProcess * sizeof(pthread_t));
    if (threads == NULL) {
